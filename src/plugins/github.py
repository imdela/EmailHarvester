...
Plugin explicitly built to interface natively using Bing and Google site-search for GitHub.
"""
...
def search(domain: str, limit: Any) -> list[str]:
    """
    Performs GitHub searching by aggregating results from Bing and Google.
    """
    all_emails = []

    bing_url = "http://www.bing.com/search?q=site%3Agithub.com+%40{word}&count=50&first={counter}"
    app_emailharvester.init_search(bing_url, domain, limit, 0, 50, "GitHub [Bing]")
    ...
    app_emailharvester.init_search(google_url, domain, limit, 0, 100, "GitHub [Google]")
    app_emailharvester.process()
    all_emails.extend(app_emailharvester.get_emails())
