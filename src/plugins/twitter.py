...
Plugin explicitly built to interface natively using Bing and Google site-search for Twitter.
"""
...
def search(domain: str, limit: int, harvester: Any) -> list[str]:
    """Executes the search and harvest sequence for Twitter via Bing and Google.
    ...
    bing_url = "http://www.bing.com/search?q=site%3Atwitter.com+%40{word}&count=50&first={counter}"
    harvester.init_search(bing_url, domain, limit, 0, 50, "Twitter [Bing]")
    harvester.process()
    all_emails.extend(harvester.get_emails())

    google_url = 'https://www.google.com/search?num=100&start={counter}&hl=en&q=site%3Atwitter.com+"%40{word}"'
    harvester.init_search(google_url, domain, limit, 0, 100, "Twitter [Google]")
    harvester.process()
    all_emails.extend(harvester.get_emails())
...
class Plugin:
    """Plugin bridge for searching emails on Twitter."""
